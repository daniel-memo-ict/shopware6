...
import { loadDIVE } from './utils/spatial-dive-load-utils';
...
export default class SpatialBaseViewerPlugin extends Plugin {
     ...
     */
    public async initViewer(force: boolean) {
        this.setReady(false);
        // @ts-ignore
        ...
        if (this._dive == undefined) {
            this._dive = await window.DIVEClass.QuickView(this.options.modelUrl, { autoStart: false, renderer: { canvas: this.el as HTMLCanvasElement}});
        }
